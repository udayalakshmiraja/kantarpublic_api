...
$dbPass='';
$uploadPath='D:/upload/';
$soapClientPath_wsdl = 'http://localhost/sos/server.php?wsdl';
...
if (empty($_GET) || isset($_GET['wsdl'])) {
    intSoapFile();
} elseif (isset($_GET['getSurveyResultStatusRequest_ext']) && $_GET['getSurveyResultStatusRequest_ext']=='list') {
    getFileList();
} elseif (isset($_GET['getDataFileRequest_ext']) &&  $_GET['getDataFileRequest_ext']=='file') {
    getDataFiles();
...
function getDataFiles()
{
    ...
    if (isset($_GET['surveyFileList'])) {
        $f=$files->fileName=$_GET['fileName'];
    }
    if (isset($_GET['surveyFileList']['fileType'])) {
